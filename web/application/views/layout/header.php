            ...
            <div class="col-md-6">
                ...
                <div id="navbar" class="navbar-collapse collapse">
                    <ul class="nav nav-justified">
                        ...
                        <li class="menu"><a class="nav-texto" href="#"> Notícias </a></li>
...
<div  class="container" id="banner" style="position: inherit">
    <div class="row">
        <div class="col-sm-7">
            ...
            <form class="form" action="<?= base_url('index.php/users/redirect') ?>" method="post">
                <div class="form-group">
                    <div class="input-group">
                        <input type="text" name="email" placeholder="E-mail" class="form-control" <?php if( $this->session->has_userdata('login') ) : echo 'disabled="disabled"'; endif ?>>
                        <div class="input-group-addon"><i class="fa fa-envelope-o"></i></div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="input-group">
                        <input type="password" name="password" placeholder="Senha" class="form-control" <?php if( $this->session->has_userdata('login') ) : echo 'disabled="disabled"'; endif ?>>
                        <div class="input-group-addon"><i class="fa fa-unlock"></i></div>
                    </div>
                </div>
                <?php if (!$this->session->has_userdata('login'))
                echo '<p><a href="#" style="color:#f2f2f2">Esqueci minha senha</a></p>';
                ?>
                <button type="submit" class="btn btn-primary " name='login' value='TRUE' <?php if( $this->session->has_userdata('login') ) : echo 'disabled="disabled"'; endif ?>> Entrar      </button>
                <button class="btn btn-primary pull-right" name='register' value='TRUE'> Cadastrar    </button>
            </form>
            <br>
        </div>
    </div>
...
<?php if ($this->session->has_userdata('login'))
    $this->load->view('layout/header_contributor'); 
?>